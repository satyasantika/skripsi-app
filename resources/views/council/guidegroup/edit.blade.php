                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('guidegroup.update',$guidegroup) }}">
                            ...
                            <div class="col-md-6">
                                <select id="lecture_id" name="lecture_id" class="form-select" aria-label="Default select example" required>
                                    @foreach ($lectures as $lecture)
                                        <option value="{{ $lecture->id }}" {{ $lecture->id === $guidegroup->lecture_id ? 'selected' : '' }}>{{ $lecture->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <input id="examinator" type="number" class="form-control" name="examinator" min="0" value="{{ $guidegroup->examinator }}">
                            </div>
                        </div>
